...
employee_blueprint = Blueprint('employee_blueprint', __name__, template_folder='../../templates/',static_folder='../../static/')
...
def display_image():
    if os.path.exists(Config.image_data_file):
        ...
        image = sorted(os.listdir(Config.upload_image_path[0]),key = lambda x: os.path.getatime(os.path.join(Config.upload_image_path[0],x)),reverse=True)
    if image:
